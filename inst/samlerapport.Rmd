...
cap <- paste("Eksempeltabell som viser", params$var, "fra mtcars.")
label <- "eksTab"

if (params$type == "html") {
  t <- rapbase::mst(tab = dt, type = "html", cap = cap, label = label)
}

if (params$type == "pdf") {
  t <- rapbase::mst(tab = dt, type = "latex", cap = cap, label = label)  
}
t
```
...
```{r eksFig, echo=FALSE, fig.cap=paste("Eksempelfigur som viser frekvensfordelingen av", params$var, "fra mtcars.")}
