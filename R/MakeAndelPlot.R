...
PlotAndelerGrVar <- function(RegData, 
  ...
  ovreGrense <- max(prettyVals, gjennomsnittY, na.rm = TRUE) # Sørg for at både maks andel og gjennomsnittslinje får plass
  ...
  # 6) Plot
  p <- ggplot2::ggplot(ggDataFrame, aes(x = gruppeNavn, y = andelerPlot))
  ...
  if (visKvalIndGrenser) {
    p <- p +
      ggplot2::geom_rect(
        data = indikatorBand,
        aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = indLevels),
        inherit.aes = FALSE,
        alpha = 0.20
      ) +
